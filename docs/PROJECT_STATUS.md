...
||||**Phase L.21 LLM-Guided Natural-Language Intake Demo:** `docs/checkpoints/phase_l21_llm_guided_natural_language_intake_demo_checkpoint.md` — 17-check conversational builder demo using mock mediator; NL-style messages (from/to/subject/ssic/body/signature/window_envelope) converted to KV lines through `MockLLMBuilderMediator`; all updates applied via `BuilderSession.ingest_user_message()`; validation, warning acceptance, finalize, PDF render + cleanup; guardrails verify no unsafe keys, no API key leakage; 17/17 L.21 PASS, 8/8 L.20 SKIP, 14/14 L.19 PASS, 15/15 L.17 PASS, 15/15 L.15 PASS, 13/13 L.12 PASS, 27/27 H.13 PASS, 11/11 K.3 PASS, 45/45 intake PASS, 18/18 H.4 PASS (265/265 total + 1 smoke SKIP); no live LLM/API/network calls; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized; recommended next phase: `Phase L.22 LLM-Guided Intake User Interface Decision`.
