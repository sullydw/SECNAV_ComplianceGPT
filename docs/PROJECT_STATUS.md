...
||||**Phase L.22 LLM-Guided Intake User Interface Decision:** `docs/planning/phase_l22_llm_guided_intake_user_interface_decision.md` — Decision document comparing CLI, Streamlit, Flask/FastAPI, and ChatGPT workflow options; hybrid recommendation chosen: Streamlit as next user-facing prototype, CLI retained for regression/operator fallback, Flask/FastAPI deferred until UX stabilizes; defines L.23 Streamlit scope (chat input, draft panel, missing fields, validation, accept/finalize/render buttons, provider status); safety requirements: UI must pass all updates through adapter + BuilderSession ingestion, no direct payload mutation, no raw API key display, no CCI/config controls; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized; recommended next phase: `Phase L.23 Streamlit LLM-Guided Intake Prototype`.
