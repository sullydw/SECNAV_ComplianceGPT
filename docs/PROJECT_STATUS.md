...
|**Phase L.30S Chat-Orchestrated Letter Builder:** `docs/checkpoints/phase_l30s_chat_orchestrated_letter_builder_checkpoint.md` — Chat-orchestrated letter builder added; commands: `start`, `chat --chat-id <ID> --text <TEXT>`, `status --chat-id <ID>`, `reset --chat-id <ID>`; deterministic intent routing: new/say/answer -> hidden say/status, revise/edit/change -> hidden revise/status, approve/looks good -> hidden approve/status, render/finalize/export/make pdf -> hidden ready/render gate, preview/show me -> hidden preview, status/ready -> hidden ready; user does not need to know new/say/preview/revise/approve/ready/render commands; chat layer delegates to `hermes_session_manager.py`; core builder/renderer/validator/candidate logic not duplicated; safety gates preserved: no approve unless preview gate met, no render unless validation_ready and approved_ready true, revisions clear approval through existing manager behavior; smoke test PASS (10 steps); output PDF tmp\chat_builder_20260703_080932.pdf (1815 bytes); no renderer/layout/validator/catalog/config changes.
...
|**Latest Checkpoint:** `docs/checkpoints/phase_l30s_chat_orchestrated_letter_builder_checkpoint.md` — Phase L.30S chat-orchestrated letter builder proven
