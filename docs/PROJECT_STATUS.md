...
||||**Phase L.22 LLM-Guided Intake User Interface Decision:** `docs/planning/phase_l22_llm_guided_intake_user_interface_decision.md` — Decision document comparing CLI, Streamlit, Flask/FastAPI, and ChatGPT workflow options; hybrid recommendation chosen: Streamlit as next user-facing prototype, CLI retained for regression/operator fallback, Flask/FastAPI deferred until UX stabilizes; defines L.23 Streamlit scope and safety requirements; no code changes; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized; recommended next phase: `Phase L.23 Streamlit LLM-Guided Intake Prototype`.
||||**Phase L.23 Streamlit LLM-Guided Intake Prototype:** `docs/checkpoints/phase_l23_streamlit_llm_guided_intake_prototype_checkpoint.md` — Streamlit app `app_streamlit_llm_guided_intake.py` wrapping BuilderSession + Adapter + Provider; chat input, draft summary panel, missing fields, validation, accept/finalize/render buttons, provider status indicator; guarded import for missing Streamlit; all updates via `BuilderSession.ingest_user_message()`; no direct payload mutation; no API key display; no renderer/CCI config controls; render delegates to `src/pdf_v6_render.py` via subprocess; mock provider default; 10/10 L.23 smoke PASS, 17/17 L.21 PASS, 8/8 L.20 SKIP, 14/14 L.19 PASS, 15/15 L.17 PASS, 15/15 L.15 PASS, 13/13 L.12 PASS, 27/27 H.13 PASS, 11/11 K.3 PASS, 45/45 intake PASS, 18/18 H.4 PASS (265/265 total + 1 smoke SKIP); no live LLM/API/network calls; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized; recommended next phase: `Phase L.24 Streamlit Prototype Usability Pass`.
||||**Phase L.24 Streamlit Prototype Usability Pass:** `docs/checkpoints/phase_l24_streamlit_prototype_usability_pass_checkpoint.md` — Usability improvements: instructions banner, example prompts with copy buttons, "🔄 New Letter" reset in sidebar, transcript/history panel, provider status explanation, improved validation display (Errors/Warnings/Advisories metric cards), button help text, raw payload in collapsible expander; all backend safety boundaries unchanged; 16/16 L.24 PASS, 10/10 L.23 smoke PASS, 17/17 L.21 PASS, 14/14 L.19 PASS, 15/15 L.17 PASS, 15/15 L.15 PASS, 27/27 H.13 PASS, 11/11 K.3 PASS, 45/45 intake PASS, 18/18 H.4 PASS (275/275 total + 1 smoke SKIP); no live LLM/API/network calls; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized; recommended next phase: `Phase L.25 Streamlit Guided Intake Manual Demo Script`.
||||**Phase L.25 Streamlit Guided Intake Manual Demo Script:** `docs/checkpoints/phase_l25_streamlit_guided_intake_manual_demo_script_checkpoint.md` — Manual demo script `docs/demo/streamlit_guided_intake_manual_demo_script.md` documenting step-by-step user walkthrough: prerequisites, launch command (`streamlit run app_streamlit_llm_guided_intake.py`), localhost:8501 URL, 8-step intake scenario, expected UI observations per step, warning/finalize/render/reset/provider behavior, troubleshooting guide, verification checklist, cleanup reminder; no backend code changes; 14/14 L.25 PASS, 16/16 L.24 PASS, 10/10 L.23 smoke PASS, 17/17 L.21 PASS, 45/45 intake PASS, 18/18 H.4 PASS (289/289 total + 1 smoke SKIP); no live LLM/API/network calls; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized; recommended next phase: `Phase L.26 Simple Streamlit Local Launcher`.
||||**Phase L.26 Simple Streamlit Local Launcher:** `docs/checkpoints/phase_l26_simple_streamlit_local_launcher_checkpoint.md` — Windows launchers `launch_secnav_streamlit.bat` and `launch_secnav_streamlit.ps1`; both try known Python path first, fall back to `python`, verify Streamlit install, print `http://localhost:8501`, run `python -m streamlit run app_streamlit_llm_guided_intake.py`; demo doc updated to mention launchers; 12/12 L.26 PASS, 14/14 L.25 PASS, 16/16 L.24 PASS, 10/10 L.23 smoke PASS, 45/45 intake PASS, 18/18 H.4 PASS (301/301 total + 1 smoke SKIP); no live LLM/API/network calls; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
||||**Phase L.26A Streamlit Pending-Decisions Hotfix:** `docs/checkpoints/phase_l26a_streamlit_pending_decisions_hotfix_checkpoint.md` — Hotfix for `TypeError: object of type 'int' has no len()` crash when `pending_decisions` was returned as an integer rather than a list; added `_has_pending_decisions()` helper to safely handle int/list/None/missing; replaced unsafe `len(val.get("pending_decisions", [])) > 0` pattern; 7/7 L.26A PASS, 12/12 L.26 PASS, 16/16 L.24 PASS, 10/10 L.23 smoke PASS, 45/45 intake PASS, 18/18 H.4 PASS (308/308 total + 1 smoke SKIP); no live LLM/API/network calls; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
||||**Phase L.26B Streamlit Debug Panel Pass:** `docs/checkpoints/phase_l26b_streamlit_debug_panel_checkpoint.md` — Added "🐛 Debug — Behind the Scenes" collapsible panel showing full mediator output (intent, explanation, confidence, proposed KV lines, updates), current validator state JSON, block reason, and warnings/errors; enables copy-paste troubleshooting back to developer; stores `last_mediator_output` in session state; resets with New Letter; 10/10 L.26B PASS, 16/16 L.24 PASS, 12/12 L.26 PASS, 10/10 L.23 smoke PASS, 45/45 intake PASS, 18/18 H.4 PASS (318/318 total + 1 smoke SKIP); no live LLM/API/network calls; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
||||**Phase L.26C Ollama Provider Manual Path:** `docs/checkpoints/phase_l26c_ollama_provider_manual_path_checkpoint.md` — Real Ollama backend `_ollama_backend(config)` using stdlib `urllib` (no hard dependency on requests/httpx); calls `localhost:11434/api/tags` for health check then `POST /api/chat` with `format: "json"` and `temperature: 0.2`; fails closed on any error (URLError, JSONDecodeError, empty content, missing server); model defaults to `llama3.2` or reads from `SECNAV_OLLAMA_MODEL`; factory injects real backend when `provider == "ollama"`; Streamlit `_provider_available()` shows "manual" for Ollama; 11/11 L.26C PASS, 10/10 L.26B PASS, 7/7 L.26A PASS, 12/12 L.26 PASS, 16/16 L.24 PASS, 10/10 L.23 smoke PASS, 45/45 intake PASS, 18/18 H.4 PASS (329/329 total + 1 smoke SKIP); no live LLM/API/network calls in default mode; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
||||**Phase L.26D One-Click Ollama Streamlit Launcher:** `docs/checkpoints/phase_l26d_one_click_ollama_streamlit_launcher_checkpoint.md` — Windows launchers `launch_secnav_streamlit_ollama.bat` and `launch_secnav_streamlit_ollama.ps1`; check Streamlit install, check Ollama reachability at `localhost:11434/api/tags` via Python stdlib `urllib`, print clear error and exit if Ollama not running, set `SECNAV_LLM_PROVIDER=ollama` and `SECNAV_OLLAMA_MODEL=llama3.2`, print provider mode and model, start Streamlit at `http://localhost:8501`; existing mock launchers unchanged; demo doc updated with Ollama launcher section; 19/19 L.26D PASS, 11/11 L.26C PASS, 10/10 L.26B PASS, 7/7 L.26A PASS, 12/12 L.26 PASS, 16/16 L.24 PASS, 10/10 L.23 smoke PASS, 45/45 intake PASS, 18/18 H.4 PASS (348/348 total + 1 smoke SKIP); no live LLM/API/network calls in default mode; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
||||**Phase L.26E Ollama Provider and Model Picker:** `docs/checkpoints/phase_l26e_ollama_provider_model_picker_checkpoint.md` — Sidebar provider/model picker with Mock / Ollama Local / Ollama Cloud options; local model discovery via `list_ollama_models()` querying `localhost:11434/api/tags`; dropdown populated with installed models; fail-closed instructions if Ollama not running; cloud placeholder shows "not yet configured"; `_run_mediation()` respects UI session state over env defaults; `list_ollama_models()` added to `src/llm_provider_config.py` using stdlib `urllib`; 18/18 L.26E PASS, 19/19 L.26D PASS, 11/11 L.26C PASS, 10/10 L.26B PASS, 7/7 L.26A PASS, 12/12 L.26 PASS, 16/16 L.24 PASS, 10/10 L.23 smoke PASS, 45/45 intake PASS, 18/18 H.4 PASS (367/367 total + 1 smoke SKIP); no live LLM/API/network calls in default mode; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
||||**Phase L.26F Ollama Inference Debug and Provider-State Fix:** `docs/checkpoints/phase_l26f_ollama_inference_debug_and_provider_state_fix_checkpoint.md` — Root cause confirmed: local Ollama `/api/tags` reachable while `/api/chat` returned HTTP 404, causing safe adapter degradation to `unknown` with no proposed KV ingestion; added centralized localhost-only inference helper `call_ollama_inference()` with `/api/chat` → `/api/generate` fallback, service-status/debug helpers, deterministic provider/model state propagation helpers, explicit `Provider Status` sidebar visibility, and provider status snapshotting for transcript/debug context; preserved mock default, BuilderSession ownership, fail-closed behavior, no renderer/layout/CCI changes, and no external network by default; 24/24 L.26F PASS, 18/18 L.26E PASS, 19/19 L.26D PASS, 11/11 L.26C PASS, 10/10 L.26B PASS, 7/7 L.26A PASS, 16/16 L.24 PASS, 27/27 H.13 PASS, 11/11 K.3 PASS, 18/18 H.4 PASS (151/151 total across rerun suites); error promotion unauthorized; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
|||||**Phase L.26G Ollama Timeout Fallback and Cold-Start Headroom:** `docs/checkpoints/phase_l26g_ollama_timeout_fallback_and_coldstart_checkpoint.md` — Fixed timeout fallback loop bug: `TimeoutError` on `/api/chat` previously hit generic `except Exception` and returned immediately instead of trying `/api/generate`; added explicit `except TimeoutError: last_error = e; continue` so both endpoints are exhausted before fail-closed; raised Ollama default timeout from 30s to 120s and capped minimum at 120s to accommodate cold model loads; added Streamlit sidebar caption warning users that first inference may take 60–90 seconds; preserved mock default, BuilderSession source-of-truth, no renderer/layout/CCI changes, no external network by default, and no secrets; 8/8 L.26G PASS plus all downstream rerun suites PASS; error promotion unauthorized; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
||||||**Phase L.26H Streamlit Launcher Dependency Auto-Install Hotfix:** `docs/checkpoints/phase_l26h_streamlit_dependency_autoinstall_checkpoint.md` — Removed unconditional `import pyperclip` from render-path button callback; added guarded top-level `try/except ImportError` with `_copy_to_clipboard()` helper returning `(bool, str)`; if `pyperclip` is missing, UI shows friendly toast instead of crashing; all four launchers (`launch_secnav_streamlit.bat`, `.ps1`, `launch_secnav_streamlit_ollama.bat`, `.ps1`) now auto-check and auto-install `streamlit` and `pyperclip` using the selected Python; re-check after install; exit cleanly with manual command on failure; no Ollama auto-install; no vendor LLM SDK auto-install; no API key display; no renderer/layout/CCI config/severity/catalog/validator/prompt/command changes; 22/22 L.26H PASS plus all downstream rerun suites PASS; error promotion unauthorized; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
|**Phase L.26I Robust Ollama Localhost Detection Hotfix:** `docs/checkpoints/phase_l26i_ollama_localhost_detection_hotfix_checkpoint.md` — Added `OLLAMA_HOSTS = ["127.0.0.1", "localhost"]` to `src/llm_provider_config.py`; default constants use `127.0.0.1`; `ollama_service_status()` probes both hosts in order and returns `active_endpoint` + `tried_endpoints`; `_discover_working_ollama_host()` helper probes both endpoints; `call_ollama_inference()` discovers working host then routes chat/generate through it; Streamlit UI shows active endpoint caption and troubleshooting expander with `curl` commands, `ollama serve`, `ollama list`, `ollama pull llama3.2`; Ollama launchers (`.bat` + `.ps1`) now probe `127.0.0.1:11434` first then `localhost:11434`, print per-endpoint status, print endpoint in use on success, print troubleshooting on failure; no silent mock fallback; no API key display; no external network; no renderer/layout/CCI changes; 24/24 L.26I PASS plus all downstream rerun suites PASS; error promotion unauthorized; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
||||||**Phase L.26J Unify Streamlit Ollama Availability Check:** `docs/checkpoints/phase_l26j_streamlit_ollama_availability_unification_checkpoint.md` — Unified Ollama availability source of truth by caching `ollama_service_status()` result in `st.session_state.ollama_status` and passing it to `provider_debug_status()` via new `cached_ollama_status` parameter; removed redundant `list_ollama_models()` call from sidebar; sidebar and mediation/debug paths now share the same single probe result, eliminating "Ollama does not appear to be running" false negatives when sidebar already showed reachable; preserved `_discover_working_ollama_host()` as independent connectivity check at actual inference time; no silent mock fallback; no API key display; no external network; no renderer/layout/CCI changes; 19/19 L.26J PASS plus all downstream rerun suites PASS; error promotion unauthorized; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
||||||**Phase L.26K Ollama Inference Endpoint Diagnostics and Repair:** `docs/checkpoints/phase_l26k_ollama_inference_endpoint_diagnostics_checkpoint.md` — Added rich per-endpoint failure diagnostics to `call_ollama_inference()`; accumulates `diagnostics` list with `endpoint`, `model`, `provider`, `timeout`, `status`, `exception_type`, `exception_message`, `http_code`, `response_snippet` for each `/api/chat` and `/api/generate` attempt; builds multi-line `explanation` summarizing provider, model, host, timeout, and per-endpoint failure details; debug panel surfaces dedicated **Ollama Inference Diagnostics** subsection when inference fails; preserves fail-closed behavior (`intent: unknown`, empty KV, confidence 0.0); no silent mock fallback; no API key display; no external network; no renderer/layout/CCI changes; 19/19 L.26K PASS plus all downstream rerun suites PASS; error promotion unauthorized; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
||||||**Phase L.26L Guided Intake Inference and Follow-Up Question Repair:** `docs/checkpoints/phase_l26l_guided_intake_inference_checkpoint.md` — Repaired `MockLLMBuilderMediator` so intake/extraction is no longer blocked by current validator errors; added `_infer_subject()` and `_infer_body()` helpers to derive plain-language subject and purpose draft from natural-language requests; added "from X to Y" crossover post-processing to split combined routing clauses; added contextual follow-up question for date-change requests when date is missing; `blocked_reason` remains `null` during intake; `requires_user_confirmation` is `true` for inferred fields; preserves mock default, BuilderSession source-of-truth, no silent Ollama fallback, no renderer/layout/CCI changes; 17/17 L.26L PASS plus all downstream reruns PASS; recommended next phase: `Phase L.26M Ollama Empty Content Response Handling`.
||||||||**Phase L.26M Ollama Empty Content Response Handling:** `docs/checkpoints/phase_l26m_ollama_empty_content_response_checkpoint.md` — Diagnosed why Ollama (`qwen3.5:9b`) returned empty `message.content` when `format: "json"` is used; added `_extract_ollama_content()` helper that tries safe alternate fields (`message.content` → `message.thinking` (JSON-guarded) → `response` → `content`) for both `/api/chat` and `/api/generate`; added `_diagnose_empty_content()` to capture response keys, message keys, done flag, and compact preview; added `_safe_response_preview()` for safe diagnostics; refactored `call_ollama_inference()` to use `_attempt()` inner function with four attempts: `chat(format_json)` → `generate(format_json)` → `chat(no_format_json)` → `generate(no_format_json)`; all attempts still require valid JSON before acceptance; fixed Ollama timeout regression by enforcing 120s minimum in `LLMProviderConfig.__init__`; updated L.26K Check D to match refactored loop; preserves mock default, localhost-only, fail-closed, BuilderSession boundary, no renderer/CCI changes; 17/17 L.26M PASS, 19/19 L.26K PASS, 17/17 L.26L PASS, 19/19 L.26J PASS, 24/24 L.26I PASS, 22/22 L.26H PASS, 8/8 L.26G PASS, 24/24 L.26F PASS, 18/18 L.26E PASS, 12/12 L.26 PASS, 16/16 L.24 PASS, 10/10 L.23 PASS, 17/17 L.21 PASS, 14/14 L.25 PASS, 27/27 H.13 PASS, 15/15 K.3 PASS, 18/18 H.4 PASS, 36/36 H.24 PASS; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
