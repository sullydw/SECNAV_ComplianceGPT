...
|**Phase L.30N-1 Preview Hardening:** `docs/checkpoints/phase_l30n1_hardening_checkpoint.md` — Candidate source warnings now read provenance from top-level fields with `resolved_value` fallback; approve blocks incomplete/build_status sessions; finalize blocks when validation `finalize_allowed` is false; manager `ready` separates `validation_ready` from `approved_ready`; preview remains read-only; no renderer/validator/catalog/config changes; no error promotion.
...
**Latest Checkpoint:** `docs/checkpoints/phase_l30p_e2e_approved_render_demo_checkpoint.md` — Phase L.30P end-to-end approved render demo proven  
