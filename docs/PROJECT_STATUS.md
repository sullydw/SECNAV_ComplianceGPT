...
## L.32Q — Natural Sender Lookup Suggestion Workflow

Added `tools/run_phase_l32q_natural_sender_lookup_suggestion_smoke.py` to
prove that a natural first-turn draft with a sender phrase like
"from MCAS New River" quietly surfaces an official lookup From/letterhead
suggestion without interrupting the draft flow.

### Verified behavior

- Natural first-turn draft extracts the literal From value.
- When official lookup is enabled and a clean source-backed From candidate
  exists, it is stored as a quiet pending suggestion while the draft flow
  continues normally.
- Complete From candidate may suggest From + complete letterhead, but does not
  auto-apply; it is applied only after `confirm candidate`.
- Incomplete From candidate suggests From only and does not invent letterhead.
- When official lookup is disabled, the natural draft still works and no
  provider call happens.
- Dismissing the candidate prevents the same sender text from immediately
  recreating the same suggestion.
- L.30Y approval/revise behavior is preserved.
- No static command database, no open-ended web search, no auto-apply.
...
- `tools/hermes_chat_builder.py`:
  - `_maybe_add_source_candidate` now bypasses the adapter's controlled-alias
    lookup suppression when it intentionally asks for a source-backed
    candidate (via a transient `bypass_controlled_alias_lookup` flag).
- `tools/official_command_lookup_adapter.py`:
  - `official_command_lookup` honors `bypass_controlled_alias_lookup` in the
    passed state, allowing the chat builder to request source-backed
    candidates even for controlled-alias phrases.
- `tools/run_phase_l32q_natural_sender_lookup_suggestion_smoke.py` (new).
...
|**Phase L.31C Callable Out-Path Pass-Through:** `tools/hermes_chat_builder.py` — `send_secnav_chat_turn(..., out=...)` now stores the provided path into chat state and `_run_render_gate` uses it; if `out` is omitted, existing path is preserved; smoke test PASS; no renderer/layout/validator/catalog/config/rule changes.
|**Phase L.31F First-Turn Intake Hardening:** `tools/hermes_chat_builder.py` + `tools/hermes_session_manager.py` — Intent classification hardened so first-turn natural-language requests containing both creation and revise keywords (e.g., "I need a letter ... make the body about ...") route as `say` intake rather than `revise`; key:value turns (from:, to:, subj:, body:, signature:, date:) routed through fixed `apply` path; first turn reaches `build_status`/`draft_preview`, preview reaches `draft_preview`, `validation_ready=True`, `approved_ready=False` until explicit approval; unsupported revise blocked without false payload changes; render blocked before approval, succeeds after approval/ready; smoke test PASS; no renderer/layout/validator/catalog/config/rule changes.
|**Phase L.31G Hermes End-to-End Tool Use Retest:** `docs/checkpoints/phase_l31g_hermes_end_to_end_tool_use_retest_checkpoint.md` — Hermes callable backend retest after L.31F fixes; realistic first-turn natural-language request accepted as `say` intent, not `revise`; key:value follow-ups for `date` and `signature` persisted correctly; preview reached `draft_preview`, `validation_ready=True`, `approved_ready=False` before approval; `looks good` approval succeeded; `make the PDF` rendered successfully to `tmp\chat_builder_20260712_163918.pdf` (1785 bytes); no interactive command used; no live lookup used; no production code changes; verdict: no remaining user-facing problems.
|**Phase L.31H First-Turn Field Extraction Quality:** `tools/hermes_chat_builder.py` — Refactored first-turn field extraction so realistic natural-language requests produce clean fields without follow-ups; `To` line no longer contaminated with trailing topic phrase; explicit `subject` preserved; body converted to usable sentence; first turn reaches `draft_preview` immediately, `validation_ready=True`; `approved_ready` still gated by explicit approval; smoke test PASS 7/7; all regressions PASS; no renderer/layout/validator/catalog/config/rule changes.
|**Phase L.31I Standard Letterhead Required:** `src/letterhead_v6_resolve.py` + `tools/hermes_secnav_tool.py` — Standard letters now require command/activity letterhead data before reaching `draft_preview` or `validation_ready`; fallback fields `letterhead_top_line`, `letterhead_activity`, `letterhead_address` supported via key:value input when `unit_identity` is absent; `_missing_for_preview()` and `_apply_standard_letterhead_gate()` enforce the gate without modifying CCI rules; preview shows LETTERHEAD block; rendered PDF includes letterhead before date/from/to/subj; `approved_ready` still gated by explicit approval; all regressions (L.31H, L.31F, L.31A, L.31C, L.31F-1, L.30U, L.30X) PASS; no renderer/layout/validator/catalog/config/rule changes.
**Phase L.31N Unnumbered Body Left Margin Alignment:** `src/pdf_v6_render.py` — Unnumbered body paragraphs now start at `left_margin_pt` (aligned under the Subj line label) instead of `left_margin_pt + 18`; numbered paragraph indentation, subparagraph indentation, subject/header rendering, letterhead, and signature unchanged; continuation lines continue to use `left_margin_pt`; smoke test PASS (L.31N 6/6); all regressions PASS (L.31M 10/10, L.31L 16/16, L.31K 19/19, L.31J 7/7, L.31I 7/7, L.31H 7/7); no validator/catalog/config/rule changes.
**Phase L.31M Suppress Nonblocking Preview NEXT ACTION:** `tools/hermes_secnav_tool.py` — Preview text NEXT ACTION section now aligns with actual readiness state; when `validation_ready=True` and `approved_ready=False`, it tells the user to review and approve instead of asking for SSIC or originator code; when approved, it points to render (`make the PDF`); when `validation_ready=False`, existing missing-field prompting is preserved; status `next_action` dict and preview NEXT ACTION no longer contradict each other; smoke test PASS (L.31M 10/10); all regressions PASS (L.31L 16/16, L.31K 19/19, L.31J 7/7, L.31I 7/7, L.31H 7/7); no renderer/layout/validator/catalog/config/rule changes.
**Phase L.31L User-Facing Prompt Cleanup:** `tools/hermes_secnav_tool.py` — Preview text now labels SSIC and Originator Code as `[OPTIONAL / IF USED]` instead of `[NEEDED]` when those fields are absent but not blocking for a standard letter; removed `[SSIC NEEDED]` and `[ORIGINATOR CODE NEEDED]` placeholders from draft_preview; next_action no longer asks for SSIC or originator code when validation_ready=True and the only missing fields are optional; required field prompting unchanged when validation_ready=False; approval gate preserved; render blocked before approval; smoke test PASS (L.31L 16/16); all regressions PASS (L.31K 19/19, L.31J 7/7, L.31I 7/7, L.31H 7/7); no renderer/layout/validator/catalog/config/rule changes.
**Phase L.31K Mixed Prose + Key:Value Intake Merge:** `tools/hermes_chat_builder.py` + `tools/run_phase_l31k_mixed_prose_keyvalue_intake_merge_smoke.py` — Mixed first-turn requests now merge prose-extracted fields with explicit key:value fields in a single turn; one-shot natural-language requests reach `draft_preview` with `validation_ready=True` without follow-up; SSIC 5216, originator code S-1, letterhead fields, body, and signature all captured and shown in preview; approval gate preserved; render succeeds after approval; smoke test PASS 19/19; all regressions PASS (L.31J 7/7, L.31I 7/7, L.31H 7/7); no renderer/layout/validator/catalog/config/rule changes.
**Phase L.31J Letterhead Authority Line Validation:** `src/letterhead_v6_resolve.py` + `tools/hermes_secnav_tool.py` — Fallback `letterhead_top_line` now validated as a required authority line; accepted values: `DEPARTMENT OF THE NAVY` (Navy activities) and `UNITED STATES MARINE CORPS` (Marine Corps activities); lowercase input normalized to uppercase; invalid authority lines (e.g., `SOME RANDOM COMMAND`) block `validation_ready` and render; `next_action` explains allowed authority lines; preview and rendered PDF show normalized authority line; all regressions (L.31I, L.31H, L.31F) PASS; no CCI rules modified.
|**Phase L.30U Interactive Chat Loop:**
**Phase L.30T Friendly Chat Responses:** `docs/checkpoints/phase_l30t_friendly_chat_responses_checkpoint.md` — Plain-English `assistant_response` field added to `tools/hermes_chat_builder.py` chat/status/reset outputs alongside stable machine-readable fields; response types covered: build_status (names next missing field and invites details), draft_preview (ready for review, approve/change), revise (updated, approval cleared, re-approve), approve (confirmed, ask to make PDF), blocked_render (explains why blocked), rendered (PDF ready with path); safety gates preserved: no render unless validation_ready and approved_ready, approval clears on revision; smoke test PASS (11 steps); no renderer/layout/validator/catalog/config changes.
**Phase L.30S Chat-Orchestrated Letter Builder:**
|**Phase L.30R Conversation Wrapper Polish:** `docs/checkpoints/phase_l30r_conversation_wrapper_polish_checkpoint.md` — hermes_session_manager.py remains a thin delegator over hermes_secnav_tool.py; wrapper commands new/say/answer/preview/approve/ready/revise/candidates/candidate-add/candidate-confirm/candidate-reject/finalize/render all verified; preview passes through mode, preview_text, render_gate, approval, pending_candidates, confirmed_candidates, next_action, message; revise passes through proposed_kv, payload_changed, approval_cleared, approval, validation_summary, message; approve passes through approved_for_finalize, approval_current, current_preview_hash, error, message; ready separates validation_ready, approved_ready, approval_required, approval_gate, approval, render_gate; smoke test PASS (15 checks); rendered wrapper smoke PDF tmp\l30r_manager_wrapper_smoke.pdf (1929 bytes); no renderer/layout/validator/catalog/config changes.
**Phase L.30P End-to-End Approved Render Demo:** `docs/checkpoints/phase_l30p_e2e_approved_render_demo_checkpoint.md` — Realistic end-to-end approved-render demo from intake through PDF render proven; workflow: start → apply realistic SECNAV letter fields → add L.30J-style official-live candidate → preview shows [OFFICIAL SOURCE] + source_url + source_title + source_limitation → approve → ready separates validation_ready and approved_ready → natural-language revise (make body more formal) clears approval → re-approve → finalize → render to PDF; approval hardening: approve blocked before preview gate, succeeds after preview gate; candidate source display and revise/approval clearing proven; demo tool `tools/run_phase_l30p_e2e_approved_render_demo.py`; output PDF ~2.0 KB; all 16 demo steps PASS; no renderer/layout/validator/catalog/config changes; no error promotion.

||**Phase L.30M Natural-Language Draft Revisions:** `docs/checkpoints/phase_l30m_natural_language_revision_checkpoint.md` — Revise command now supports deterministic natural-language revisions while preserving existing key:value behavior; supported phrases: remove attachment sentence, change subject to ..., change signer to ..., change date to ..., change to/from line to ..., make body more direct/formal, shorten body; unsupported instructions fail safely with clear message; no broad LLM/freeform rewriting; no candidate changes, live lookup, finalize, or render inside revise; approval clears when draft-relevant payload changes; unsupported instructions do not clear approval; targeted smoke tests passed.
||**Phase L.30L Preview Readability Polish:** `docs/checkpoints/phase_l30l_preview_readability_checkpoint.md` — Preview output readability improved while keeping all gate/approval/candidate behavior unchanged; BUILD STATUS gains clearer section headers (KNOWN FIELDS, MISSING / BLOCKING ITEMS, PENDING CANDIDATES, CONFIRMED SOURCE-BACKED FACTS, APPROVAL STATUS, NEXT ACTION); DRAFT PREVIEW gains restructured sections (DOCUMENT HEADER, ADDRESSES, SUBJECT, BODY, SIGNATURE, VALIDATION SUMMARY, candidate/source sections, approval status); body review label improved to "BODY  [AI-DRAFTED OR USER-PROVIDED — REVIEW REQUIRED]"; preview gate, approval hash, finalize/render gate, candidate behavior, and read-only guarantee unchanged; targeted smoke tests passed.
||**Phase L.30K Gated Preview and Final Review Loop:** `docs/checkpoints/phase_l30k_gated_preview_final_review_checkpoint.md` — Complete functional milestone: L.30K-1 gated read-only preview command (build_status vs draft_preview), L.30K-2 preview approval state and hash-based stale detection, L.30K-3 revise command clears approval when draft-relevant payload changes, L.30K-4 finalize/render blocked unless current preview is approved; approval gate enforced in `tools/hermes_secnav_tool.py` and `tools/hermes_session_manager.py`; all targeted smoke tests passed; no renderer/layout/config/validator/catalog changes; no error promotion.
||**Phase L.30J Complete Letter with Official-Live Candidate Confirmation:** `docs/checkpoints/phase_l30j_official_live_candidate_workflow_checkpoint.md` — End-to-end proven: complete letter request → agent-browser CDP lookup of official `.mil` source (`https://www.newriver.marines.mil/`) → candidate creation with `source_tier=official_live` → explicit user confirmation → `unit_identity` applied only after confirmation → ready=true, can_render=true → finalize → render to PDF; session `builder_20260627_125640`, candidate `cand_20260627_125659_514dc4`; zero errors/warnings/advisories; no auto-application before confirm; no code/renderer/validator/config changes; no static database.
...
|||**Latest Checkpoint:** `docs/checkpoints/phase_l31a_hermes_callable_secnav_tool_interface_checkpoint.md` — Phase L.31A Hermes callable SECNAV tool interface proven
|||**Latest Checkpoint:** `docs/checkpoints/phase_l30z_first_time_user_quickstart_checkpoint.md` — Phase L.30Z first-time user quickstart polish proven
**Phase L.30Z First-Time User Quickstart Polish:** `docs/USER_QUICKSTART.md` — Startup JSON clarified as ignorable technical session info; `chat_id` reassured as not needed for normal use; minimal complete example added (single-prompt field provision); `show me the draft` added to plain-English command list; safety gates section mentions early-render blocked message; PDF path explicitly noted as printed after successful render; sample conversation includes early-render being blocked before approval; no production code, renderer, layout, validator, catalog, config, or rule changes.
...
||||||||**Phase L.29H General Live Lookup Candidate Workflow Design:** `docs/checkpoints/phase_l29h_general_live_lookup_candidate_workflow_checkpoint.md` — ... [same content]
...
|||||||||**Phase L.30J Complete Letter with Official-Live Candidate Confirmation:** `docs/checkpoints/phase_l30j_official_live_candidate_workflow_checkpoint.md` — End-to-end proven: complete letter request → agent-browser CDP lookup of official `.mil` source (`https://www.newriver.marines.mil/`) → candidate creation with `source_tier=official_live` → explicit user confirmation → `unit_identity` applied only after confirmation → ready=true, can_render=true → finalize → render to PDF; session `builder_20260627_125640`, candidate `cand_20260627_125659_514dc4`; zero errors/warnings/advisories; no auto-application before confirm; no code/renderer/validator/config changes; no static database; recommended next phase: TBD.
|
||||||||**Phase L.29Q Hermes Agent Session Manager:** `docs/checkpoints/phase_l29q_session_manager_checkpoint.md` — Created `tools/hermes_session_manager.py`, a thin deterministic session-manager coordinator that wraps `tools/hermes_secnav_tool.py` CLI commands to make multi-turn Hermes-driven SECNAV correspondence sessions easier to run; commands: `new`, `resume`, `say`, `next`, `answer`, `ready`, `summary`; all delegate to existing `start`, `ingest`, `next-action`, `apply`, `status` CLI handlers; JSON output by default; no renderer/validator/detector/candidate/BuilderSession logic duplicated; regression runner `tools/run_phase_l29q_session_manager_regression.py` (29 checks) PASS; L.29P/L.29O/L.29M/L.29L/L.29K/L.29C/L.28 regressions all PASS; no renderer/layout/config/validator/catalog changes; no static database; error promotion unauthorized; recommended next phase: TBD.
