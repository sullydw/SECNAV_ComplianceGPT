...
|**Phase L.31F-1 Key:Value Apply Persistence Fix:** `tools/hermes_session_manager.py` — Added missing `apply` subcommand delegating to `hermes_secnav_tool.py apply`; key:value turns (from:, to:, subj:, body:, signature:, date:) now persist directly into session payload instead of silently failing; `tools/hermes_chat_builder.py` intent hardening preserved; safety gates (approval, render gate, unsupported revise blocking) unchanged; smoke test PASS; no renderer/layout/validator/catalog/config/rule changes.
