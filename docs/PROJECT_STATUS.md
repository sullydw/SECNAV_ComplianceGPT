...
||||**Phase L.26G Ollama Timeout Fallback and Cold-Start Headroom:** `docs/checkpoints/phase_l26g_ollama_timeout_fallback_and_coldstart_checkpoint.md` — Fixed L.26F bug where `TimeoutError` on `/api/chat` hit the generic `except Exception` handler and returned immediately instead of falling back to `/api/generate`; added explicit `except TimeoutError as e: last_error = e; continue` so both endpoints are tried before failing closed; raised Ollama default timeout from 30s to 120s in `LLMProviderConfig.__init__` and capped minimum at 120s to accommodate cold model loads; added Streamlit sidebar caption warning users that first inference may take 60–90 seconds; preserved mock default, BuilderSession source-of-truth, no renderer/layout/CCI changes, no external network by default, and no secrets; 8/8 L.26G PASS, all downstream rerun suites PASS (same totals as L.26F); error promotion unauthorized; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
