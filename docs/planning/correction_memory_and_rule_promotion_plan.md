...
**Next Phase:** `Phase L.6  Conversational Builder Payload-to-PDF Dry Run` — Wire `finalize()` output into existing `pdf_v6_render.py` via safe isolated entry point; ensure payload normalization produces renderable JSON; add dry-run validation that payload schema matches renderer expectations; do not modify renderer layout logic; only validate payload compatibility.
