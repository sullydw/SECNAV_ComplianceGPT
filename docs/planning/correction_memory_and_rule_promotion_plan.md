...
||**Next Phase:** `Phase L.18 Real LLM Provider Selection and Adapter Plan` — Evaluate and select a real LLM provider (OpenAI, Anthropic, local llama.cpp, etc.) for integration behind the adapter; define concrete backend callable, prompt refinement strategy, cost/latency guardrails; no active LLM calls in L.18 — only selection and planning; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized.
