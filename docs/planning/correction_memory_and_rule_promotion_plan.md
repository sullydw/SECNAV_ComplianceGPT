...
||**Next Phase:** `Phase L.19 LLM Provider Interface and Configuration Plumbing` — Define concrete configuration plumbing for LLM provider selection (config keys, env var patterns, provider registry); implement provider-agnostic backend wrapper callable; add optional smoke-test harness for OpenAI/Ollama behind feature flag; all regressions continue using fake backend; no live API calls by default; no API keys committed; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized.
