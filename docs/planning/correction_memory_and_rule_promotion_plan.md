...
||**Next Phase:** `Phase L.21 LLM-Guided Natural-Language Intake Demo` — Use the mock mediator (or future live mediator) to guide a user conversationally through creating a complete Navy/Marine Corps letter; prove end-to-end flow from natural-language input to structured payload + PDF through the adapter layer; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized.
