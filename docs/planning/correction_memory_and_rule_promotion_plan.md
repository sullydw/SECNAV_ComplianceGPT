...
||**Next Phase:** `Phase L.20 Optional Live LLM Smoke-Test Harness` — Add optional, feature-flagged smoke-test harness that can call a real LLM (OpenAI or Ollama) behind the adapter for a single end-to-end letter creation; all standard regressions continue using the fake backend; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized.
