...
|**Next Phase:** `Phase L.9 Conversational Builder Question Text and Signature Capture Improvements` — Improve builder question text and add structured signature capture (name/role/title/authority) to CLI; keep key-value ingestion; no NL parsing; no renderer/config/validator/catalog/command changes; error promotion unauthorized.
