...
||**Phase L.26A Streamlit Pending-Decisions Hotfix:** `docs/checkpoints/phase_l26a_streamlit_pending_decisions_hotfix_checkpoint.md` — Hotfix for `TypeError: object of type 'int' has no len()` crash when `pending_decisions` was returned as an integer rather than a list; added `_has_pending_decisions()` helper to safely handle int/list/None/missing; replaced unsafe `len(val.get("pending_decisions", [])) > 0` pattern; 7/7 L.26A PASS; no live LLM/API/network calls; no renderer/layout/config/validator/catalog/command changes; error promotion unauthorized; recommended next phase: `Phase L.27 Streamlit Launcher Manual Verification`.
