#!/usr/bin/env python3
"""
CCI Subject-Line Regression Runner

Runs the CCI subject-line validator against pass, fail, and warning fixtures.
...
def run_validator(root: Path, fixture: str, expected_pass: bool, expect_warnings: bool = False) -> bool:
    print("=" * 72)
    label = f"cci_subject_validate on {fixture}"
...
def main() -> int:
    ...
    print("CCI SUBJECT-LINE REGRESSION RUNNER")
    ...
    results.append(run_validator(root, "audit_cci_subject_valid.json", expected_pass=True, expect_warnings=False))
    results.append(run_validator(root, "audit_cci_subject_invalid.json", expected_pass=False, expect_warnings=False))
    results.append(run_validator(root, "audit_cci_subject_warning.json", expected_pass=True, expect_warnings=True))

    print("=" * 72)
    if all(results):
        print("CCI SUBJECT-LINE REGRESSION RESULT: PASS")
        return 0
    else:
        print("CCI SUBJECT-LINE REGRESSION RESULT: FAIL")
        return 1
